...
public class Enemy2 : LivingEntity  //Enemy is a LivingEntity so it inherits all LivingEntity functions
{
    [Header("Public")]
...
/*                    else
                ...
                if (playerDetected == true)
                {
                    ...
                    {
                        PlayAnimation("Enemy2_seek1", 0.25f);
                    ...
                    else
                    {
                        if (remainingDistance <= attack2Range)
                        {
                            if (currentState != State.Attack)
                            {
                                PlayAnimation("Enemy2_idle1", 0.25f);
    ...
    {
        PlayAnimation("Enemy2_attack1", 0.1f);
    ...
    private void Attack1Damage()
    {
        ...
        GameObject slash = Instantiate(attack1Area, attack1Muzzle.transform.position, attack1Muzzle.transform.rotation);
        Destroy(slash, 3f);
    }

    private IEnumerator Attack2()
    {
        PlayAnimation("Enemy2_attack2", 0.1f);
        AudioSource.PlayClipAtPoint(attack2Sound, transform.position, 0.20f);
        currentState = State.Attack;
        navMeshAgent.speed = 0;
        yield return new WaitForSeconds(1f); //Animation duration
    ...
    {
        GameObject groundslam = Instantiate(attack2Area, attack2Muzzle.transform.position, attack2Muzzle.transform.rotation);
        groundslam.transform.Find("Collider").GetComponent<Groundslam>().damage = attack2Damage;
