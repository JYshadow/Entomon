...
public class Enemy7 : LivingEntity  //Enemy is a LivingEntity so it inherits all LivingEntity functions
{
    [Header("Public")]
...
/*                    else
                ...
                if (playerDetected == true)
                {
                    ...
                    if (remainingDistance <= attack2Range && attack2Enabled == true)
                    {
                    ...
                    {
                        PlayAnimation("Enemy7_seek1", 0.25f);
                    ...
                    else
                    {
                        ...
                        else
                        {
                            if (currentState != State.Attack)
                            {
                                PlayAnimation("Enemy7_idle1", 0.25f);
    ...
    {
        PlayAnimation("Enemy7_attack1", 0.1f);
        AudioSource.PlayClipAtPoint(attack1Sound, transform.position, 0.10f);
    ...
    {
        PlayAnimation("Enemy7_attack2", 0.1f);
        AudioSource.PlayClipAtPoint(attack2Sound, transform.position, 0.10f);
        currentState = State.Attack;
        navMeshAgent.speed = 0;
    ...
    {
        animator.speed = 1.5f;
        minMsBetweenAttacks /= 2f;
        maxMsBetweenAttacks /= 2f;
        attack2Enabled = false;
        moveSpeed *= 2f;
        transform.Find("Enrage").GetComponent<ParticleSystem>().Play();
        ...
        attack2Enabled = true;
        moveSpeed /= 2f;
        transform.Find("Enrage").GetComponent<ParticleSystem>().Stop();
    }
}