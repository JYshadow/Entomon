...
public class Enemy1 : LivingEntity  //Enemy is a LivingEntity so it inherits all LivingEntity functions
{
    [Header("Public")]
    ...
    public Material materialEnrage;
    ...
    Transform target;
    ParticleSystemRenderer animParticleSystemRenderer;
...
/*    float undetectDuration = 5f;
    ...
    protected override void Start() //Override the Livingentity start
    {
        ...
        animParticleSystemRenderer = particleSystemObj.gameObject.GetComponent<ParticleSystemRenderer>();
        particleMain = particleSystemObj.gameObject.GetComponent<ParticleSystem>().main;
        ...
        animParticleSystemRenderer.material = materialBase;
...
/*                    else
                ...
                if (playerDetected == true)
                {
                    ...
                    {
                        PlayAnimation("Enemy1_seek1", 0.25f);
                    ...
                    else
                    {
                        if (remainingDistance <= attack2Range)
                        {
                            ...
                            {
                                PlayAnimation("Enemy1_seek1", 0.25f);
                                particleMain.simulationSpeed = 4f;
                        ...
                        else
                        {
                            if (currentState != State.Attack)
                            {
                                PlayAnimation("Enemy1_idle1", 0.25f);
                                particleMain.simulationSpeed = 1.5f;
    ...
    IEnumerator Wake()
    {
        awake = false;
        navMeshAgent.speed = 0;
        particleMain.simulationSpeed = 3f;  //Spawn at a higher speed
        yield return new WaitForSeconds(2); //After x seconds slow down speed and become awake
        awake = true;
        navMeshAgent.speed = moveSpeed;
        particleMain.simulationSpeed = 1.5f;
    }
    ...
    {
        PlayAnimation("Enemy1_attack1", 0.1f);
        AudioSource.PlayClipAtPoint(attack1Sound, transform.position, 0.25f);
    ...
    {
        PlayAnimation("Enemy1_attack2", 0.1f);
        AudioSource.PlayClipAtPoint(attack2Sound, transform.position, 0.15f);
        currentState = State.Attack;
        navMeshAgent.speed = 0;
        particleMain.simulationSpeed = 6f;
        animParticleSystemRenderer.material = materialEnrage;
        yield return new WaitForSeconds(1.5f); //Animation duration
