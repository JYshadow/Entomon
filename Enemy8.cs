...
public class Enemy8 : LivingEntity  //Enemy is a LivingEntity so it inherits all LivingEntity functions
{
    [Header("Public")]
...
/*                    else
                ...
                if (playerDetected == true)
                {
                    ...
                    {
                        PlayAnimation("Enemy8_seek1", 0.25f);
                    ...
                    else
                    {
                        if (remainingDistance <= attack2Range)
                        {
                            if (currentState != State.Attack)
                            {
                                PlayAnimation("Enemy8_idle1", 0.25f);
    ...
    {
        PlayAnimation("Enemy8_attack1", 0.1f);
        AudioSource.PlayClipAtPoint(attack1Sound, transform.position, 0.1f);
    ...
    {
        PlayAnimation("Enemy8_attack2", 0.1f);
        AudioSource.PlayClipAtPoint(attack2Sound, transform.position, 0.2f);
        currentState = State.Attack;
        navMeshAgent.speed = 0;
    ...
    {
        GameObject areaSlam = Instantiate(attack2Area, attack2Muzzle.transform.position, attack2Muzzle.transform.rotation);
        areaSlam.GetComponent<AreaSlam>().damage = attack2Damage;
    }
}