...
public class Enemy5 : LivingEntity  //Enemy is a LivingEntity so it inherits all LivingEntity functions
{
    [Header("Public")]
...
/*                    else
                ...
                if (playerDetected == true)
                {
                    ...
                    {
                        PlayAnimation("Enemy5_seek1", 0.25f);
                    ...
                    else
                    {
                        if (remainingDistance <= attack2Range)
                        {
                            if (currentState != State.Attack)
                            {
                                PlayAnimation("Enemy5_idle1", 0.25f);
    ...
    {
        PlayAnimation("Enemy5_attack1", 0.1f);
    ...
    {
        //target.gameObject.GetComponent<LivingEntity>().TakeDamage(attack1Damage * levelManager.difficultyMultiplier);
        GameObject stinger = Instantiate(attack1Area, attack1Muzzle.transform.position, attack1Muzzle.transform.rotation);
        stinger.GetComponent<Stinger>().damage = attack1Damage;
    }

    private IEnumerator Attack2()
    {
        PlayAnimation("Enemy5_attack2", 0.1f);
    ...
    {
        GameObject sludge = Instantiate(attack2Area, attack2Muzzle.transform.position, attack2Muzzle.transform.rotation);
        sludge.GetComponent<Sludge>().damage = attack2Damage;
    }
}